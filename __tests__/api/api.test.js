...
describe('PetCare API Integration Tests', () => {
  ...
  describe('Authentication API', () => {
    ...
    test('POST /auth/register - Register new user', async () => {
      ...
      if (response.success) {
        ...
        expect(response.user.correo).toBe(TEST_CONFIG.testUser.email.toLowerCase());
    ...
    test('POST /auth/login - Login with invalid credentials', async () => {
      const response = await authApi.login({
        email: TEST_CONFIG.testUser.email,
    ...
    test('GET /auth/me - Get current user with valid token', async () => {
      if (!authToken) {
    ...
    test('GET /auth/me - Get current user with invalid token', async () => {
      const response = await authApi.getMe('invalid_token_12345');

      console.log('❌ Invalid Token Response:', JSON.stringify(response, null, 2));
  ...
  describe('Pet Management API', () => {
    ...
    test('POST /pets - Create new pet', async () => {
      if (!authToken) {
        console.log('⚠️  No token available, skipping test');
        return;
      }
      ...
      console.log('🐕 Create Pet Response:', JSON.stringify(response, null, 2));
    ...
    test('GET /pets - List all pets', async () => {
      if (!authToken) {
        console.log('⚠️  No token available, skipping test');
        return;
      }
      ...
      console.log('📋 List Pets Response:', JSON.stringify(response, null, 2));
    ...
    test('GET /pets/:id - Get single pet', async () => {
      if (!authToken) {
        console.log('⚠️  No token available, skipping test');
        return;
      }
    ...
    test('PUT /pets/:id - Update pet', async () => {
      ...
      console.log('✏️  Update Pet Response:', JSON.stringify(response, null, 2));
    ...
    test('DELETE /pets/:id - Delete pet', async () => {
      ...
      console.log('🗑️  Delete Pet Response:', JSON.stringify(response, null, 2));
    ...
    test('POST /pets - Create pet with invalid data', async () => {
      if (!authToken) {
        console.log('⚠️  No token available, skipping test');
        return;
      }
      ...
      console.log('❌ Invalid Pet Response:', JSON.stringify(response, null, 2));
  ...
  describe('Document Upload API', () => {
    ...
    test('POST /upload_document.php - Upload document (mocked)', async () => {
      ...
      console.log('⚠️  Document upload test requires manual testing or E2E setup');
  ...
  describe('Logout', () => {
    test('POST /auth/logout - Logout user', async () => {
      if (!authToken) {
        console.log('⚠️  No token available, skipping test');
        return;
      }
